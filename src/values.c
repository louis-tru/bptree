				   ...
				   bp_value_t *value)
{
	...
	value->_prev_offset = ntohll_(*(uint64_t *) (buff));
	value->_prev_length = ntohll_(*(uint64_t *) (buff + 8));
				   ...
				   uint64_t *length)
{
	...
	if (previous != NULL) {
		*(uint64_t *) (buff) = htonll_(previous->offset);
		*(uint64_t *) (buff + 8) = htonll_(previous->length);
	} else {
		*(uint64_t *) (buff) = 0;
		*(uint64_t *) (buff + 8) = 0;
	}
