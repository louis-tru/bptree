...
TEST_START("range get test", "range")
  ...
  for (i = 1; i < n; i++) {
    key[5] = i;
    value[5] = i;
    assert(bp_set_s(&db, key, value) == BP_OK);
  }

  matched = 0;

  /* try getting 1 key-value */
  bp_get_range_s(&db, "key: \x15", "key: \x15", range_cb, &matched);
  ...
  bp_get_range_s(&db, "key: \x05", "key: \x1f", range_cb, &matched);
  ...
  bp_get_range_s(&db, "key: \x12", "key: \x5f", range_cb, &matched);
  ...
  bp_get_range_s(&db, "key: \x01", "key: \xfa", range_cb, &matched);
